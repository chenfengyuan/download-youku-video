...
class Atom:
    def __init__(self, type_, size, body):
        assert len(type_) == 4
        self.type = type_
        self.size = size
        self.body = body
...
class CompositeAtom(Atom):
    def __init__(self, type_, size, body):
        ...
        Atom.__init__(self, type_, size, body)
...
class VariableAtom(Atom):
    def __init__(self, type_, size, body, variables):
...
def read_raw(stream, size, left, type_):
    assert size == left + 8
...
def read_atom(stream):
    ...
    if type in atom_readers:
        func = atom_readers[type]
        return func(stream, size, left, type)
